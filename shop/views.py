...
def shop(request):
    ...
    purchases = Purchase.objects.filter(user=user).select_related('item').order_by('-valid_until')
...
def buy_auto_water(request):
    ...
    if user.ton_balance < ton_price:
        return JsonResponse({"status": "error", "message": _("Недостаточно TON")}, status=400)

    item = ShopItem.objects.filter(type='auto_water', duration=hours).first()
    if not item:
        return JsonResponse({"status": "error", "message": _("Товар не найден")}, status=400)
    ...
    return JsonResponse({
        "status": "success",
        "message": _("Автополив %(hours)sч добавлен в инвентарь! Активировать можно с этой страницы или на дереве.") % {
            "hours": hours
        }
    })
...
def buy_shop_item(request, item_id):
    ...
    if item.price_token_type == 'CF':
        if user.cf_balance < price:
            return JsonResponse({"status": "error", "message": _("Недостаточно FL")}, status=400)
    ...
    valid_until = timezone.now() + datetime.timedelta(hours=item.duration) if item.duration else None
    Purchase.objects.create(user=user, item=item, price_paid=price, valid_until=valid_until)

    messages.success(request, _("%(item)s куплен и добавлен в инвентарь!") % {"item": item.name})
...
def use_shop_item(request, purchase_id):
    ...
    if not tree_id:
        ...
        if trees.count() == 1:
            ...
            return JsonResponse({"status": "error", "message": _("Выберите дерево для применения.")})
    else:
    ...
    if purchase.valid_until and timezone.now() > purchase.valid_until:
        ...
        return JsonResponse({
            "status": "error",
            "message": _("Срок действия предмета истёк. Предмет удалён из инвентаря.")
        })

    # ---- Автополив ----
    if purchase.item.type == 'auto_water':
        if tree.auto_water_until and tree.auto_water_until > timezone.now():
            return JsonResponse({"status": "error", "message": _("Автополив уже активен!")})
        hours = purchase.item.duration or 24
        tree.auto_water_until = timezone.now() + datetime.timedelta(hours=hours)
        tree.save(update_fields=["auto_water_until"])
        purchase.delete()
        return JsonResponse({
            "status": "success",
            "message": _("Автополив активирован на %(hours)sч!") % {"hours": hours}
        })

    # ---- Удобрение ----
    if purchase.item.type == 'fertilizer':
        if tree.fertilized_until and tree.fertilized_until > timezone.now():
            return JsonResponse({"status": "error", "message": _("Удобрение уже активно!")})
        hours = purchase.item.duration or 24
        tree.fertilized_until = timezone.now() + datetime.timedelta(hours=hours)
        tree.save(update_fields=["fertilized_until"])
        purchase.delete()
        return JsonResponse({
            "status": "success",
            "message": _("Дерево удобрено (доход ×2)!")
        })

    return JsonResponse({"status": "error", "message": _("Неизвестный предмет.")})


@require_POST
def buy_fertilizer(request):
    user = get_current_user(request)
    tree_id = request.POST.get("tree_id")  # пока не используешь — ок
    ...
    return JsonResponse({"status": "success", "message": _("Удобрение успешно куплено!")})
...
def buy_branches(request):
    ...
    if user.ton_balance < ton_price:
        return JsonResponse(
            {"status": "error", "message": _("Недостаточно TON для покупки веток.")},
            status=400
        )
    ...
    return JsonResponse({
        "status": "success",
        "message": _("Куплено веток: %(qty)s. Теперь у вас %(total)s веток.") % {
            "qty": quantity,
            "total": user.branches_balance
        }
    })
...
def buy_ton_tree(request):
    ...
    if Tree.objects.filter(user=user, type='TON').exists():
        messages.warning(request, _("У вас уже есть TON-дерево."))
        return redirect("home")

    cost_ton = Decimal("5")
    if user.ton_balance < cost_ton:
        messages.error(request, _("Недостаточно TON для покупки TON-дерева."))
        return redirect("home")
    ...
    messages.success(request, _("TON-дерево успешно куплено!"))
    return redirect("home")
