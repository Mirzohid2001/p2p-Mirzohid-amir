...
class User(models.Model):
    ...
    ton_wallet = models.CharField(max_length=64, blank=True, null=True, unique=True)
...
class TonDepositRequest(models.Model):
    ...
    def __str__(self):
        return f"{self.user} — {self.amount} TON — {self.memo} — {'OK' if self.is_completed else 'WAIT'}"

class TonProcessedTx(models.Model):
